...
const opts1 = {
  ...
  path: `/rest/v1/profiles?select=id,full_name,about_me&gender=eq.Female&limit=1`,
  method: 'GET',
  headers: {
    'apikey': ANON_KEY,
...
https.get(opts1, (res) => {
  ...
  res.on('end', () => {
    ...
    console.log(`\nTesting female ID: ${id}`);
    console.log(`Current name: ${profile.full_name}`);
    console.log(`Current about_me: ${profile.about_me}\n`);
    
    const updateData = {
      full_name: 'Test Update',
      age: 28,
      about_me: 'This is a test bio update',
    ...
    const req = https.request(opts2, (res) => {
      ...
      res.on('end', () => {
        ...
        if (respData) console.log(`Response: ${respData}`);
