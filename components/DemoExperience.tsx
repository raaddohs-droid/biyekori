...
export default function DemoExperience() {
  ...
  const tabStyle = (t: string): React.CSSProperties => ({
    ...
    color: tab===t ? '#B89255' : 'rgba(253,246,238,0.5)',
    fontSize: '12px', cursor: 'pointer', fontFamily: 'system-ui',
    fontWeight: tab===t ? 600 : 400, transition: 'all 0.2s'
  })

  return (
    <div style={{ background:'#4B1421', borderRadius:'20px', padding:'24px', color:'#FDF6EE', maxWidth:'480px', margin:'0 auto', border:'1px solid rgba(240,192,64,0.15)' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:'12px', background:'rgba(240,192,64,0.05)', border:'1px solid rgba(240,192,64,0.12)', borderRadius:'12px', padding:'14px', marginBottom:'16px' }}>
        <div style={{ width:'50px', height:'50px', borderRadius:'50%', background:'#6D1F2F', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'22px', flexShrink:0 }}>🧕</div>
        <div>
      ...
      {tab==='call' && (
        <div>
          ...
          {(callState==='idle'||callState==='ended') && (
            <button onClick={startCall} style={{ width:'100%', padding:'14px', background:'#6D1F2F', border:'none', borderRadius:'10px', color:'white', fontSize:'15px', fontWeight:600, cursor:'pointer', fontFamily:'system-ui' }}>
              {callState==='idle' ? '📞 Call Sumaiya' : '📞 Call Again'}
            </button>
          )}
          {callState==='ended' && (
            <Link href="/register" style={{ display:'block', marginTop:'12px', padding:'13px', background:'#6D1F2F', borderRadius:'8px', color:'#080604', fontSize:'14px', fontWeight:700, textDecoration:'none', textAlign:'center', fontFamily:'system-ui', letterSpacing:'1px' }}>
      ...
      {tab==='chat' && (
        <div>
          <div ref={chatRef} style={{ height:'210px', overflowY:'auto', padding:'12px', background:'rgba(0,0,0,0.2)', borderRadius:'10px', marginBottom:'12px', display:'flex', flexDirection:'column', gap:'8px' }}>
            {messages.map((m,i) => (
              <div key={i} style={{ maxWidth:'82%', padding:'9px 13px', borderRadius:m.from==='them'?'12px 12px 12px 4px':'12px 12px 4px 12px', fontSize:'14px', lineHeight:1.55, background:m.from==='them'?'rgba(240,192,64,0.1)':'#6D1F2F', color:'#FDF6EE', alignSelf:m.from==='them'?'flex-start':'flex-end', fontFamily:'system-ui' }}>
                {m.text}
              </div>
            ))}
            {isTyping && (
          ...
          {!chatEnded ? (
            <div style={{ display:'flex', gap:'8px' }}>
              ...
              <button onClick={sendMsg} disabled={isTyping} style={{ padding:'10px 16px', background:'#6D1F2F', border:'none', borderRadius:'8px', color:'white', fontSize:'14px', cursor:'pointer', fontFamily:'system-ui', opacity:isTyping?0.5:1 }}>Send</button>
            </div>
          ) : (
            <Link href="/register" style={{ display:'block', padding:'13px', background:'#6D1F2F', borderRadius:'8px', color:'#080604', fontSize:'14px', fontWeight:700, textDecoration:'none', textAlign:'center', fontFamily:'system-ui', letterSpacing:'1px' }}>
        ...
        <div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
            {Object.entries(LABELS).map(([key,label]) => (
              <button key={key} onClick={()=>!gamePick&&setGamePick(key)} style={{ padding:'12px 16px', background:gamePick===key?'rgba(219,39,119,0.15)':gamePick?'rgba(240,192,64,0.02)':'rgba(240,192,64,0.04)', border:`1px solid ${gamePick===key?'#6D1F2F':'rgba(240,192,64,0.15)'}`, borderRadius:'8px', color:gamePick&&gamePick!==key?'rgba(253,246,238,0.38)':'rgba(253,246,238,0.85)', fontSize:'14px', cursor:gamePick?'default':'pointer', textAlign:'left', fontFamily:'system-ui', transition:'all 0.2s' }}>
                {label}
              </button>
            ))}
          </div>
          {gamePick && (
            <div style={{ background:'rgba(240,192,64,0.07)', border:'1px solid rgba(240,192,64,0.2)', borderRadius:'10px', padding:'16px', marginTop:'14px', fontFamily:'system-ui', fontSize:'14px', lineHeight:1.75 }}>
              <div style={{ marginBottom:'8px' }}><span style={{ color:'#B89255', fontWeight:600 }}>Sumaiya chose:</span> {LABELS[SUMAIYA_PICK[gamePick]]}</div>
              <div style={{ marginBottom:'12px' }}><span style={{ color:'#B89255', fontWeight:600 }}>Compatibility:</span> <span style={{ fontSize:'22px', fontWeight:700, color:COMPAT[gamePick][SUMAIYA_PICK[gamePick]]>=80?'#10b981':'#f59e0b' }}>{COMPAT[gamePick][SUMAIYA_PICK[gamePick]]}%</span></div>
              ...
              <Link href="/register" style={{ display:'block', padding:'13px', background:'#6D1F2F', borderRadius:'8px', color:'#080604', fontSize:'14px', fontWeight:700, textDecoration:'none', textAlign:'center', letterSpacing:'1px' }}>
