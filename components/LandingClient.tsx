...
function HoverCard({ children, href, style }: { children: React.ReactNode, href?: string, style?: React.CSSProperties }) {
  ...
  const base: React.CSSProperties = { ...style, transform: hovered ? 'translateY(-5px)' : 'translateY(0)', boxShadow: hovered ? '0 16px 40px rgba(240,192,64,0.13)' : 'none', transition: 'transform 0.25s ease, box-shadow 0.25s ease, border-color 0.25s ease', borderColor: hovered ? 'rgba(240,192,64,0.35)' : ((style as any)?.borderColor || 'rgba(240,192,64,0.1)'), cursor: 'pointer' }
    ...
    <>
        ...
        <div style={{ display: 'flex', gap: '48px', flexWrap: 'wrap', justifyContent: 'center', padding: '0 24px 40px' }}>
          {[{ target: 1811, label: 'Women registered' }, { target: 1199, label: 'Men registered' }, { target: 3010, label: 'Total profiles' }].map(({ target, label }) => (
            <div key={label} style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '13px', color: 'rgba(253,246,238,0.65)', letterSpacing: '1px', marginTop: '4px', fontFamily: 'system-ui, sans-serif' }}>{label}</div>
            </div>
          ))}
        </div>
      </FadeIn>
      ...
      <section style={{ padding: 'clamp(60px,8vw,100px) clamp(24px,6vw,80px)', borderTop: '1px solid rgba(240,192,64,0.08)' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          <FadeIn>
            <p style={{ fontSize: '14px', color: 'rgba(240,192,64,0.85)', letterSpacing: '3px', marginBottom: '16px', textAlign: 'center', fontFamily: 'system-ui, sans-serif', textTransform: 'uppercase' }}>Biyekori is for</p>
            <h2 style={{ fontSize: 'clamp(28px, 3.5vw, 48px)', fontWeight: 700, color: '#FDF6EE', textAlign: 'center', margin: '0 0 56px', lineHeight: 1.2 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '20px' }}>
            {[
              ...
              <FadeIn key={title} delay={i * 80}>
                <HoverCard href={href} style={{ background: 'rgba(240,192,64,0.04)', border: '1px solid rgba(240,192,64,0.1)', borderRadius: '16px', padding: '28px 24px' }}>
                  ...
                  <p style={{ fontSize: '15px', color: 'rgba(253,246,238,0.75)', lineHeight: 1.75, margin: '0 0 16px', fontFamily: 'system-ui, sans-serif' }}>{desc}</p>
                  <span style={{ fontSize: '13px', color: '#F0C040', fontFamily: 'system-ui, sans-serif' }}>Get started \u2192</span>
                </HoverCard>
              </FadeIn>
            ))}
          </div>
      ...
      <section style={{ padding: 'clamp(60px,8vw,100px) clamp(24px,6vw,80px)', borderTop: '1px solid rgba(240,192,64,0.08)' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '64px', alignItems: 'center' }}>
          <FadeIn>
            <p style={{ fontSize: '14px', color: 'rgba(240,192,64,0.85)', letterSpacing: '3px', marginBottom: '16px', fontFamily: 'system-ui, sans-serif', textTransform: 'uppercase' }}>The feature nobody else has</p>
            ...
            <p style={{ fontSize: 'clamp(16px, 1.4vw, 18px)', color: 'rgba(253,246,238,0.80)', lineHeight: 1.9, margin: '0 0 28px', fontFamily: 'system-ui, sans-serif' }}>
          ...
          <FadeIn delay={150}>
            <div style={{ background: 'linear-gradient(135deg, rgba(240,192,64,0.07), rgba(240,192,64,0.02))', border: '1px solid rgba(240,192,64,0.2)', borderRadius: '20px', padding: '28px', fontFamily: 'system-ui, sans-serif' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', paddingBottom: '16px', borderBottom: '1px solid rgba(240,192,64,0.12)' }}>
                <div>
                  <div style={{ fontSize: '13px', color: 'rgba(253,246,238,0.55)', letterSpacing: '1px', marginBottom: '4px' }}>WHY YOU MATCH</div>
                  <div style={{ fontSize: '16px', color: 'rgba(253,246,238,0.88)', fontWeight: 600 }}>Sumaiya, 26 \u00b7 Dhaka</div>
                </div>
              ...
              {[
                ...
                <div key={label} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 12px', borderRadius: '10px', marginBottom: '7px', background: match ? 'rgba(16,185,129,0.08)' : 'rgba(249,115,22,0.08)', border: `1px solid ${match ? 'rgba(16,185,129,0.2)' : 'rgba(249,115,22,0.2)'}` }}>
                  ...
                  <span style={{ fontSize: '13px', color: 'rgba(253,246,238,0.88)', width: '68px', fontWeight: 600 }}>{label}</span>
                  <span style={{ fontSize: '11px', color: 'rgba(253,246,238,0.55)', flex: 1 }}>You: {you} \u00b7 Profile: {them}</span>
                  <span style={{ fontSize: '14px' }}>{match ? '\u2705' : '\u274c'}</span>
                  <span style={{ fontSize: '11px', color: 'rgba(253,246,238,0.42)', width: '34px', textAlign: 'right' }}>{pts}</span>
                </div>
              ))}
              <div style={{ marginTop: '14px', textAlign: 'center' }}>
                <span style={{ fontSize: '12px', color: 'rgba(253,246,238,0.42)' }}>Based on 13 compatibility factors</span>
              </div>
            </div>
          </FadeIn>
        </div>
      ...
      <section style={{ padding: 'clamp(60px,8vw,100px) clamp(24px,6vw,80px)', borderTop: '1px solid rgba(240,192,64,0.08)' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
          <FadeIn>
            <p style={{ fontSize: '14px', color: 'rgba(240,192,64,0.85)', letterSpacing: '3px', marginBottom: '16px', textAlign: 'center', fontFamily: 'system-ui, sans-serif', textTransform: 'uppercase' }}>What makes Biyekori different</p>
            <h2 style={{ fontSize: 'clamp(28px, 3vw, 44px)', fontWeight: 700, color: '#FDF6EE', textAlign: 'center', margin: '0 0 56px', lineHeight: 1.2 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '20px' }}>
            {[
              ...
              <FadeIn key={title} delay={i * 60}>
                <HoverCard href={href} style={{ padding: '28px 24px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(240,192,64,0.08)', borderRadius: '16px' }}>
                  ...
                  <p style={{ fontSize: '14px', color: 'rgba(253,246,238,0.72)', lineHeight: 1.85, margin: '0 0 14px', fontFamily: 'system-ui, sans-serif' }}>{desc}</p>
                  <span style={{ fontSize: '13px', color: 'rgba(240,192,64,0.75)', fontFamily: 'system-ui, sans-serif' }}>Learn more \u2192</span>
                </HoverCard>
              </FadeIn>
            ))}
          </div>
      ...
      <section style={{ padding: 'clamp(60px,8vw,100px) clamp(24px,6vw,80px)', borderTop: '1px solid rgba(240,192,64,0.08)', textAlign: 'center' }}>
        <div style={{ maxWidth: '800px', margin: '0 auto' }}>
          <FadeIn>
            <p style={{ fontSize: '14px', color: 'rgba(240,192,64,0.85)', letterSpacing: '3px', marginBottom: '16px', fontFamily: 'system-ui, sans-serif', textTransform: 'uppercase' }}>Safety and trust</p>
            <h2 style={{ fontSize: 'clamp(28px, 3vw, 44px)', fontWeight: 700, color: '#FDF6EE', margin: '0 0 16px', lineHeight: 1.2 }}>
              Your family&apos;s trust<br/><span style={{ color: '#F0C040' }}>is not negotiable.</span>
            </h2>
            <p style={{ fontSize: 'clamp(16px, 1.4vw, 18px)', color: 'rgba(253,246,238,0.78)', lineHeight: 1.9, margin: '0 0 52px', fontFamily: 'system-ui, sans-serif' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '16px' }}>
            {[
              ...
              <FadeIn key={title as string} delay={i * 70}>
                <HoverCard href={href as string} style={{ padding: '22px 14px', background: 'rgba(240,192,64,0.04)', border: '1px solid rgba(240,192,64,0.1)', borderRadius: '12px' }}>
                  <div style={{ fontSize: '28px', marginBottom: '10px' }}>{icon}</div>
                  <div style={{ fontSize: '14px', fontWeight: 700, color: '#FDF6EE', marginBottom: '5px' }}>{title as string}</div>
                  <div style={{ fontSize: '12px', color: 'rgba(253,246,238,0.58)', fontFamily: 'system-ui, sans-serif' }}>{sub as string}</div>
                </HoverCard>
              </FadeIn>
            ))}
          </div>
      ...
      <section style={{ padding: 'clamp(60px,8vw,100px) clamp(24px,6vw,80px)', borderTop: '1px solid rgba(240,192,64,0.08)' }}>
        <FadeIn>
          <div style={{ maxWidth: '700px', margin: '0 auto', textAlign: 'center' }}>
            ...
            <p style={{ fontSize: '13px', color: 'rgba(253,246,238,0.45)', letterSpacing: '2px', fontFamily: 'system-ui, sans-serif' }}>THE BIYEKORI TEAM \u00b7 DHAKA, BANGLADESH</p>
          </div>
        </FadeIn>
      </section>
    </>
